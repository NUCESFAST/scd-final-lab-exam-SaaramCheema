...
const Home = () => {
  ...
  const createClass = async () => {
    ...
    await axios.post('http://localhost:1226/create_class', {
      userId, title
    });
  }
  ...
  const joinClass = async () => {
    ...
    await axios.post('http://localhost:1226/add_class', {
      userId, classId
    });
  };

  const fetchClasses = async () => {
    const res = await axios.get('http://localhost:1226/get_classes/' + userId);
